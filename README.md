...
- dense local real graph path 연결
  ...
  - geometry: ATS road GeoJSON polyline + synthetic bidirectional edges
...
- straight/light-turn에서는 coarse public graph보다 분명히 좋아졌고
- route confidence도 올라갔지만
- turn-heavy에서는 여전히 `MATCH_LOST`가 많이 남았다
...
해석:
- straight/light-turn에서는 dense local geojson이 coarse보다 낫다.
- turn-heavy에서는 route confidence는 개선됐지만 `MATCH_LOST`는 아직 많다.

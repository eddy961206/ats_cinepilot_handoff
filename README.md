...
- `shared_memory_v2` absolute pose 계약 확정
...
- `SCSTelemetrySharedv2_ats` live decode 성공
- first ATS-backed shadow mode 성공

아직 확인 안 된 건 아래다.
- absolute world pose 복원
- `scs-sdk-controller` 기반 실제 control write
- HUD 실제 캘리브레이션
- active mode

지금 선택된 live telemetry 경로는 `SCSTelemetrySharedv2_ats` shared memory고, 이건 `configs/live_probe_moza_shared_memory.yaml`에 반영돼 있어.

이번 세션에서 실제로 확인한 결론은 이거야.
- ATS는 설치돼 있고 실행도 됐음
- `atssharedplugin64v2.dll`도 로드됐음
- `SCSTelemetrySharedv2_ats` mapping이 실제로 열렸음
- direct reader도 구현됐음
- 다만 현재 pose는 velocity 적분 기반 relative pose라서 full-quality map alignment는 아직 미완성임
...
- `docs/SHARED_MEMORY_V2_DESIGN.md`  
  현재 지원하는 MOZA shared_memory_v2 계약 메모
- `docs/RUNBOOK.md`  
  반복 실험 루프
- `docs/SAFETY.md`  
  자동주행 해제 기준
